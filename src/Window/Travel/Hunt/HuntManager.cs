...
namespace OregonTrailDotNet.Window.Travel.Hunt
{
    ...
    public sealed class HuntManager : ITick
    {
        ...
        public bool TryShoot()
        {
            ...
            // Check if the player outright missed their target. The chance to miss (as a percent) grows with how
            // long the animal has been targeted and is higher for a less capable hunter - a banker (profession 1)
            // misses three times as often as a farmer (profession 3) for the same shot. A snap shot (TargetTime 0)
            // never misses here. The old test compared 100*Random.Next() against ((int)Profession - 13)*TargetTime,
            // which is always <= 0, so the branch was dead code and profession had no effect on hunting at all.
